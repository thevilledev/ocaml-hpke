...
The source holds 13 vectors, all in Base mode. Two, X25519 with TurboSHAKE128
and X448 with TurboSHAKE256, use a KDF this library does not implement and a
KEM of RFC 9180, and are left out. The corpus keeps the other eleven:
...
- As `draft_vectors`, the four SHAKE vectors, in full: P-256 with SHAKE128
  and AES-128-GCM, P-384 with SHAKE256 and AES-256-GCM, MLKEM768-P256 with
  SHAKE128 and AES-256-GCM, and MLKEM768-X25519 with SHAKE256 and
  ChaCha20-Poly1305. The one-stage KDFs exist only in the key schedule of
  `draft-ietf-hpke-hpke`, so these run through `Hpke.Draft_hpke_04` alone. The
  test also frames `CombineSecrets_OneStage` itself over the public
  `Draft_hpke_04.Kdf.derive`, which makes the published `key`, `base_nonce` and
  `exporter_secret` known answers for the framing.
- As `kem_only_vectors`, the ML-KEM-1024 vector that uses TurboSHAKE256. An
...
selects exactly six HKDF vectors, one for each KEM, four SHAKE vectors, and
one KEM-only vector.

`Hpke.Draft_hpke_04` is also run on the six HKDF vectors and on the 64 Base and
PSK vectors of the RFC 9180 corpus, which are vectors of
`draft-ietf-hpke-hpke-04` as well.
